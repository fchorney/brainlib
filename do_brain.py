...
def main():
    ...
    messages = map(str.strip, map(str, args.messages.split(',')))
    ...
    if imgur_link is not None:
        print "Uploaded to Imgur: %s" % imgur_link
...
def parse_args():
    ...
    parser.add_argument(
        'messages',
        type=str,
        help='comma delimited list of messages (no more than 10)',
    )
    ...
    imgur_api.add_argument(
        ...
        action='store_true',
        help=(
            'set this flag to upload to imgur '
            '[must supply client id and secret]'
        ),
    )
